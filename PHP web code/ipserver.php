...
$myFile = fopen($theFile, "w"); //open to write and append
...
$toWrite = $data . PHP_EOL;
$result = fwrite($myFile, $toWrite); //write out new PPN

if ( !$result) {
	echo "Error writing ip.txt</br>";
	fclose($myFile);
	unlockIt($lockFile);
	header("Location: https://www.techlabs.nl/experiments/test.html");
	exit(-1);
